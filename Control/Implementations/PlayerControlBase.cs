...
namespace Player
{
    public sealed class PlayerControlBase : IPlayerControl
    {
        ...
        void IPlayerControl.Move()
        {
            ...
            {
                var directionMoveX = Mathf.Clamp(InputService.AxisMouseY, -2.0f, 2.0f) * _currentSpeed / 5 * Time.deltaTime;
                var directionMoveY = Mathf.Clamp(InputService.AxisMouseX, -2.0f, 2.0f) * _currentSpeed / 5 * Time.deltaTime;

                _directionMove = new Vector3(-directionMoveX, 0.0f, -directionMoveY);

                Debug.Log(_directionMove);
            }
            else
            {
                if (Input.GetKey(InputService.KeyLeftShift))
